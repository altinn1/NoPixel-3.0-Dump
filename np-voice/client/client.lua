...
function UpdateContextVolume(context, volume)
    ...
    if context == "phone" then
      TriggerEvent("np:fiber:voice-event", 'volumePhone', volume)
    end
...
-- IsTalkingProximity, ProximityTimeout = false

-- function StartProximityVoiceTask()
--     if not IsTalkingProximity then
--         IsTalkingProximity = true

--         Citizen.CreateThread(function()
--             while IsTalkingProximity do
...
--     if ProximityTimeout then
--         ProximityTimeout:resolve(false)
--     end
-- end

-- function StopProximityVoiceTask()
--     if not IsTalkingProximity or ProximityTimeout then return end

--     ProximityTimeout = TimeOut(1300):next(function (continue)
--         ProximityTimeout = nil

--         if not continue then return end

--         IsTalkingProximity = false
--     end)

--     return ProximityTimeout
-- end
...
Citizen.CreateThread(function()
    -- exports["np-keybinds-1"]:registerKeyMapping("", "Voice", "Proximity PTT", "+proximityVoice", "-proximityVoice")
    -- RegisterCommand('+proximityVoice', StartProximityVoiceTask, false)
    -- RegisterCommand('-proximityVoice', StopProximityVoiceTask, false)

    exports["np-keybinds-1"]:registerKeyMapping("", "Voice", "Proximity / Range Toggle", "+cycleProximity", "-cycleProximity", "Z")
    RegisterCommand('+cycleProximity', CycleVoiceProximity, false)
    RegisterCommand('-cycleProximity', function() end, false)

    -- needs to use ^^ system - RegisterKeyMapping('mumble', "Reset Mumble VOIP", 'keyboard', "")
