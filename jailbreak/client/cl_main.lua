...
Citizen.CreateThread(function()
    exports["np-polytarget"]:AddBoxZone("prison_services", vector3(1832.04, 2579.93, 46.01), 1.0, 1.0, {
        heading=0,
        minZ=45.81,
        maxZ=47.21
      })
...
function JailIntro(name,years,cid,date)
    ...
    SetEntityCoords(PlayerPedId(), 472.92, -1011.57, 25.28)
    SetEntityHeading(PlayerPedId(), 180.0)
    Citizen.Wait(1500) 
    DoScreenFadeIn(500)
    TriggerEvent("np-propattach:attach", "police_id_board", "attachPropPoliceIdBoard")
...
local MenuData = {
    {
        ...
        description = "Check your your remaining jail time",
        action = "np-jailbreak:handler",
        key = { type = "checkTimeRemaining" }
    },
    {
        title = "Character switch",
        description = "Go bowling with your cousin",
        children = {
            { title = "Yes", action = "np-jailbreak:handler", key = { swap = true, type = "changeCharacter"} },
            { title = "No", action = "np-jailbreak:handler", key = { swap = false, type = "changeCharacter" } },
        }
    }
}

RegisterNetEvent('np-jailbreak:services')
AddEventHandler('np-jailbreak:services', function(pArgs, pEntity, pContext)
    exports["np-ui"]:showContextMenu(MenuData)
end)

RegisterUICallback("np-jailbreak:handler", function(data, cb)
    local eventData = data.key
    if eventData.type and eventData.type == "checkTimeRemaining" then
...
AddEventHandler('TimeRemaining', function(TimeRemaining, release)
    ...
    if TimeR < 0 and release then 
       imjailed = false
        ...
        TriggerEvent("givePhone")
        SetEntityCoords(playerPed, 1841.89, 2594.42,46.02)
        TriggerEvent("chatMessage", "DOC | " , { 33, 118, 255 }, "You have no months remaining!")
        return
    end
