...
local activeLabs = {
  { milo = "gabz_biker_interior_placement_interior_2_biker_dlc_int_ware01_milo_", coords = vector3(1068.39,-2317.51,22.33) },
  { milo = "gabz_biker_interior_placement_interior_2_biker_dlc_int_ware01_milo_", coords = vector3(369.05,3562.01,25.39) },
  { milo = "gabz_biker_interior_placement_interior_2_biker_dlc_int_ware01_milo_", coords = vector3(2297.82,4849.06,33.91) },
  -- testing
...
AddEventHandler("np-inventory:itemUsed", function(item, info)
    ...
    if not exports["np-inventory"]:hasEnoughOfItem("qualityscales", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to weigh with...")
        return
    end
    if not exports["np-inventory"]:hasEnoughOfItem("emptybaggies", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to pack with...")
        return
    end
    local finished = exports["np-taskbar"]:taskBar(1000, "Packing")
    if finished ~= 100 then return end
    RPC.execute("np-meth:packCuredMeth", json.decode(info))
...
Citizen.CreateThread(function()
    ...
    local rv = exports["np-npcs"]:RegisterNPC(data)
end)
