...
function dwGetLimitTitleString()
  ...
  return "($" .. format_int(limits.min) .. " / $" .. format_int(limits.max) .. ")"
end
        ...
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false and not uiIntShown then
...
Citizen.CreateThread(function()
    while true do
        if drawCurrentHand and (prevCurrentHand ~= currentHand or prevDealersHand ~= dealersHand) then -- @HERE
          ...
          -- print(tostring(currentHand), tostring(dealersHand))
...
--                 end
--             end
--             if IsControlJustPressed(0, 201) then --Submit bet [Enter] -- @HERE
...
AddEventHandler("Blackjack:successBlackjackBet",function()
    ...
    dwWaitForBet(false) -- waitingForBetState = false
...
Citizen.CreateThread(function()
    while true do
        ...
        if sittingAtBlackjackTable and canExitBlackjack then
            ...
            if IsControlJustPressed(0, 202) and not waitingForSitDownState then  --Leave seat [ESC]
        ...
        if waitingForStandOrHitState and sittingAtBlackjackTable and blackjackGameInProgress then  -- @HERE
    ...
    while true do 
        if closeToCasino then
            ...
            for i=0,((#cfg.blackjackTables+1)*4)-1,1 do
                -- if prevClosestChair ~= closestChair then
                --   -- print("prev closestChair = ",prevClosestChair,closestChair)
                --   prevClosestChair = closestChair
                -- end
                ...
                ---- print(i, vectorOfBlackjackSeat, distToBlackjackSeat)
            end
...
RegisterUICallback("np-ui:blackJackBet", function(data, cb)
  ...
  currentBetAmount = tonumber(data.values.amount)
  local tableId = blackjack_func_368(closestChair)
  local limits = cfg.blackjackTables[tableId].limits
...
function dwWaitForBet(act)
  ...
  if waitingForBetState then
    ...
    exports['np-ui']:openApplication('textbox', {
      callbackUrl = 'np-ui:blackJackBet',
      key = 1,
      items = {
        {
          icon = "dollar-sign",
