...
Citizen.CreateThread(function()
  exports["np-keybinds-1"]:registerKeyMapping("", "Vehicle", "Cruise Control", "+vehCruise", "-vehCruise", "X")
  RegisterCommand('+vehCruise', vehCruise, false)
  RegisterCommand('-vehCruise', function() end, false)
  
  exports["np-keybinds-1"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
...
function medicRevive()
	if not inmenus and (isMedic or isDoctor) then
		TriggerEvent("revive")
	end
end
...
Citizen.CreateThread( function()
	...
	exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "Cuff / Radar Front", "+policeCuff", "-policeCuff", "UP")

	RegisterCommand('+medicRevive', medicRevive, false)
	RegisterCommand('-medicRevive', function() end, false)
	exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "EMS Revive", "+medicRevive", "-medicRevive")

	RegisterCommand('+emsHeal', emsHeal, false)
	RegisterCommand('-emsHeal', function() end, false)
	exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "EMS Heal", "+emsHeal", "-emsHeal")

	RegisterCommand('+policeEscort', policeEscort, false)
	RegisterCommand('-policeEscort', function() end, false)
	exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "Escort / Speedo", "+policeEscort", "-policeEscort", "LEFT")

	RegisterCommand('+policeSeat', policeSeat, false)
	RegisterCommand('-policeSeat', function() end, false)
	exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "Force into Vehicle", "+policeSeat", "-policeSeat", "RIGHT")

	RegisterCommand('+policeUnCuff', policeUnCuff, false)
	RegisterCommand('-policeUnCuff', function() end, false)
	exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "UnCuff / Radar Rear", "+policeUnCuff", "-policeUnCuff", "DOWN")

	RegisterCommand('+policeSoft', policeSoft, false)
	RegisterCommand('-policeSoft', function() end, false)
	exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "Soft Cuff", "+policeSoft", "-policeSoft")
end)
...
AddEventHandler("police:rob", function(pArgs, pEntity)
  ...
  local finished = exports["np-taskbar"]:taskBar(15000,"Robbing",false,true,nil,false,nil,5)
...
Citizen.CreateThread(function()
  exports["np-keybinds-1"]:registerKeyMapping("", "Gov", "(EMS) Examine Target", "+examineTarget", "-examineTarget")
end)
...
function getVehicleInDirection(coordFrom, coordTo)
	...
	if distance > 25 then
		...
		if not IsPedAPlayer(vehDriver) and not hasCarAnyPlayerPassengers(vehicle) then
			return vehicle
		end

		if isVehicle and (not IsVehicleSeatFree(vehicle, -1) or GetVehicleNumberOfPassengers(vehicle) ~= 0) then
...
local emsVehicleListWhite = {
	{"Ambulance Speedo", "emsnspeedo"},
	{"Firetruck", "firetruk"},
	{"Coroner Speedo", "emsv"},
	{"AW139 Helicopter", "emsaw139"},
