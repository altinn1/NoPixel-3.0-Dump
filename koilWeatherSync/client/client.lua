...
function checkAids()
	...
	for i = 1, #tempaidsareas do
		local distance = #(vector3(tempaidsareas[i]["x"],tempaidsareas[i]["y"],tempaidsareas[i]["z"]) - vector3(plyCoords["x"], plyCoords["y"], plyCoords["z"]))
		if distance < 350.0 then
			returninfo = true
		end
	end
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if enableSync and not insidebuilding and not inhotel and not robbing and not inhouse and not spawning and not insideSpawn then NetworkOverrideClockTime( synctime.h, synctime.m, synctime.s ) end
