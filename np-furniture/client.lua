...
objectCategories = {
    ...
    [17] = { ["category"] = "special", ["name"] = "Special" },    
}
...
function GenerateObjectLists()
    ...
    if progressionData >= 1 then   
        for i = 1, #special do
            SendNUIMessage({newOption = true, category = "special", objectvar = special[i]["object"], objectname = special[i]["name"] })
        end
    end
...
function updateDatabase(house_id)
    TriggerServerEvent("UpdateFurniture",house_id,modifiedObjects)
end

RegisterNetEvent("openFurnitureConfirm")
AddEventHandler("openFurnitureConfirm", function(house_id2,furniture)
    house_id = house_id2
    local oldobjects = modifiedObjects
