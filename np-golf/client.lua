...
AddEventHandler('np-binds:keyEvent', function(name,onDown)
	...
	if name == "general" then
		...
		if golfplaying and golfstate == 1 then
			dropShot()
		end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then 
			...
			if golfplaying then
				DisplayHelpText("Press ~g~[General]~s~ to end golf.") 
			else
				DisplayHelpText("Press ~g~[General]~s~ to start golf.") 
			end
		end
...
function MoveToBall()
	...
	while golfstate == 1 do
		...
		DisplayHelpText("Move to your ball, press ~g~[General]~s~ to ball drop if you are stuck.") 
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Press ~g~[General]~s~ to drop here.")	
		else
			DisplayHelpText("Press ~g~[General]~s~ to drop - ~r~ too far from ball or to close to hole.") 
		end
	end
end
