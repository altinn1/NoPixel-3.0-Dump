...
function startCheckpointsHandler()
  Citizen.CreateThread(function()
    ...
    TriggerServerEvent("mkr_racing:updatePositionInfo", curRace.id, totalDist, straightDistToCheckpoint, curCheckpointIndex)
      ...
      if dist < checkpointRadius and (not config.vehicleOnlyCheckpoints or isInVehicle()) then
        ...
        if curCheckpointIndex > numOfCheckpoints then -- If finish checkpoint is hit
          ...
          if curLap > curRace.laps then -- race finished
            RPC.execute("mkr_racing:finishedRace", curRace.id, bestLapTime)
            break
          end
        ...
        if config.nui.hud.showPosition then TriggerServerEvent("mkr_racing:updatePositionInfo", curRace.id, totalDist, straightDistToCheckpoint, curCheckpointIndex) end
